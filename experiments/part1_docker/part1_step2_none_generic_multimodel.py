"""Launch both equal-activity generic-peer arms on the previous nine target models.

64 samples per arm/model, eight shards of eight. CONC=40 concurrent Daytona
sandboxes, each 4 CPU / 8 GiB, matching the previous multimodel launch gate.
Per-attempt directories prevent retry results from being silently mixed.
"""
...
LOG = ROOT / os.environ.get("LOG_ROOT", "logs/260911_step2_none_generic_multimodel")
MODELS = {
...
def run_one(arm, alias, shard):
    ...
    for attempt in range(start, start + RETRIES):
        ...
        cmd = [
            ...
            "-T", "world=protein_none_generic",
            "-T", f"collab={arm == 'collab'}", "-T", "temperature=1.0",
            "-T", "turn_limit=100",
...
def main():
    ...
    jobs = [(arm, alias, shard) for shard in range(1, 9)
            for alias in MODELS for arm in ("baseline", "collab")]
    manifest = {
        "world": "protein_none_generic", "models": MODELS, "samples_per_cell": 64,
        "shards_per_cell": 8, "samples_per_shard": 8, "concurrent_shards": CONC,
        "arms": ["baseline", "collab"], "judge_model": "openrouter/anthropic/claude-opus-4.8:nitro",
        "jobs": jobs,
    }
    (LOG / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    print(f"Launching {len(jobs)} shard jobs; concurrency={CONC}; 1152 samples", flush=True)
